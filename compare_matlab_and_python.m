...
for i = 1:53
    ...
    for p = [5, 9, 10]
        ...
        assert(norm(P{p, i}.J - M{p, i}.J) / norm(P{p, i}.J) <= 1e-15, ['different J for X0 for prob: ' int2str(i) ', ' char(probtypes(p))]);
        assert(norm(P{p, i}.G - M{p, i}.G') / norm(P{p, i}.G) <= 1e-15, ['different G for X0 for prob: ' int2str(i) ', ' char(probtypes(p))]);
    end
end
