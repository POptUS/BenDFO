...
def set_constants():
    ...
    y3 = jnp.array([34780., 28610., 23650., 19630., 16370., 13720., 11540., 9744., 8261., 7030.,
                   6005., 5147., 4427., 3820., 3307., 2872.])
    y4 = jnp.array([0.844, 0.908, 0.932, 0.936, 0.925, 0.908, 0.881, 0.85, 0.818, 0.784, 0.751,
                   0.718, 0.685, 0.658, 0.628, 0.603, 0.58, 0.558, 0.538, 0.522, 0.506, 0.49,
                   0.478, 0.467, 0.457, 0.448, 0.438, 0.431, 0.424, 0.42, 0.414, 0.411, 0.406])
    y5 = jnp.array([
...
def dfovec_jax(m, n, x, nprob):
    ...
    if nprob == 1:  # Linear function - full rank.
        ...
        fvec = jnp.array([
            -c13 + x[0] + ((5 - x[1]) * x[1] - 2) * x[1],
            -c29 + x[0] + ((1 + x[1]) * x[1] - c14) * x[1]
        ])
        ...
        correction = jnp.array([0. if i % 2 == 0 else 1 / ((i + 1) ** 2 - 1) for i in range(m)])
        ...
        fvec = fvec.at[6].set(x[0]*x[4]*(x[4]**2 - 3*x[6]**2) + x[2]*x[6]*(x[6]**2 - 3*x[4]**2) + x[1]*x[5]*(x[5]**2 - 3*x[7]**2) + x[3]*x[7]*(x[7]**2 - 3*x[5]**2) + 12.6)
        fvec = fvec.at[7].set(x[2]*x[4]*(x[4]**2 - 3*x[6]**2) - x[0]*x[6]*(x[6]**2 - 3*x[4]**2) + x[3]*x[5]*(x[5]**2 - 3*x[7]**2) - x[1]*x[7]*(x[7]**2 - 3*x[5]**2) - 9.48)
