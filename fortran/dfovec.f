     subroutine dfovec(n,x,fvec,nprob)
     ...
     real*8 x(n), fvec(q)
      ...
      integer i,j
      real*8 dx,one,prod,dasum,temp,tmp1,tmp2,tmp3,tmp4,zero
      real*8 y(11),y1(15),y2(11),y3(16),y4(33),y5(65),y6(8)
     ...
     *      -1.26d1,9.48d0/
      ...
      if (nprob .eq. 1) then
         ...
         dasum = zero
         do j = 1, n
            dasum = dasum + dble(j)*x(j)
         end do
         do i = 1, q
            fvec(i) = dble(i)*dasum - one
         end do
